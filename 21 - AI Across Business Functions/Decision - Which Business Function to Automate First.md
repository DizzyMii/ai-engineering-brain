...
> **Decision:** which business function gets the first AI deployment: support, sales, marketing, legal, HR, finance ops, or internal knowledge work. **Default for the 80% case:** pick the highest-volume, human-reviewed, easily reversed task on the back-office side of [[Concept - The Front-Office Back-Office Adoption Split]] that has a clean data source. Skip the highest-visibility customer-facing one. Documentation and internal-analysis copilots (medical scribes, internal LLM assistants) score highest. Autonomous external actions (unsupervised outbound sales, unreviewed customer resolution) score lowest and should wait until the copilot version is measured and mature.
...
Restated as five scoring axes: **volume**, **error tolerance**, **reversibility**, **review cost**, **data readiness**. A function that fails an early gate (regulatory exposure, no reviewer, low volume) drops out before the differentiating axes even matter. The flowchart eliminates; it doesn't compute a weighted average, because one hard blocker (Annex III exposure, no reversibility) overrides a good score on everything else.

## Tradeoff matrix

The full per-function numbers are in [[Reference - AI Impact by Business Function]]. Below is the decision-relevant cut, one archetype row per function.
...
Sort by mode, not by department. Two functions with the same label ("customer support") can sit at opposite ends of this table depending on whether the AI drafts (copilot) or acts (autopilot); see [[Concept - Copilot vs Autopilot Deployment Modes]]. The rows are archetypes. The real scoring happens per task within a function.

## What flips the decision

**A regulated function drops to the bottom regardless of volume.** Hiring, credit and medical-diagnosis decisions are high-risk under [[Reference - The EU AI Act for Operators]] (Annex III), which requires bias audits, technical documentation and human oversight before an operator can ship them. That applies even when a human nominally makes the final call, if the AI's output materially influences it (E3, EU AI Act Annex III; enforcement pushed to Dec 2027 by the 2026 Digital Omnibus, but the compliance target doesn't move, and building without it now means rebuilding later). So HR/recruiting screening sits near the bottom of the matrix despite high volume. [[Breakdown - AI in Recruiting and HR Screening]] covers the mechanism.

**The highest-visibility function is usually the wrong first bet. It maximizes liability more than value.** Klarna's 2024 launch is the standard case. 2.3M conversations a month sounds like a win, and the company claimed an estimated $40M profit improvement, but that was Klarna's own internal estimate (E2). It had automated a broad slice of customer-facing support with no quality guardrail next to the savings metric. In May 2025 CEO Sebastian Siemiatkowski said publicly the company "went too far" and that cost-cutting had degraded service quality, and Klarna began rehiring human agents for nuanced and premium support. [[Lore - The Klarna Reversal and Support Bot Walk-Backs]] and [[Concept - Support Deflection Economics]] explain why deflection and true resolution are different numbers. Klarna didn't undo the deployment. It corrected the scope of autonomy after shipping without measuring re-contact rate. The general lesson: automate what demos well last, not first.

**A function with a clean, structured knowledge base and an existing human approval step jumps the queue, however unglamorous.** MIT's 2025 "GenAI Divide" study (52 executive interviews, 153 leader surveys, analysis of 300 public deployments) found 95% of enterprise generative-AI pilots produced no measurable P&L impact. It traced the failures mainly to organizational integration gaps, not model capability (E2, MIT Media Lab/NANDA, Aug 2025, widely reported e.g. Fortune). The same study found more than half of measured genAI budget went to sales and marketing tools, the visible, demo-friendly bets, while the biggest realized ROI came from back-office automation: cutting BPO spend and external agency costs. The function that gets the budget and the function that returns the value are frequently different, and that mismatch is a resourcing choice, not a technology limit.

**Reversibility, more than error rate alone, decides whether an axis disqualifies.** A wrong scribe draft costs a physician a few seconds of editing before signing. A wrong autonomous outbound email or booking has already left the building. That's why AI SDR autonomy stays contested even at model quality similar to scribe copilots. The 11x.ai episode (fabricated logos, unauthorized use of customer names, 70–80% churn against reported ARR) is as much a review-cost failure as a capability one: nobody checked outputs before they reached real inboxes. [[Pattern - Human-in-the-Loop Review Workflow]] shows where to insert the review step cheaply instead of skipping it.

**Data readiness is usually the hidden blocker, more than the model.** A function that scores well on volume, tolerance and reversibility still fails if its knowledge base is stale, unstructured or scattered across systems. It's the top concrete cause behind the pilot failure rate above (mechanism in [[Concept - The Pilot-to-Production Gap]]). Fixing it before automating is cheaper than discovering it in production.

**Once a function clears the gates, measure the pilot on re-contact, edit rate or error rate before scaling. A demo isn't a measurement.** [[Playbook - Picking Profitable AI Use Cases]] covers the portfolio-level version of the same discipline. [[Reference - The 2026 Navigation Cheatsheet]] gives the current state of play across functions, to sanity-check a choice against what's been tried elsewhere.
