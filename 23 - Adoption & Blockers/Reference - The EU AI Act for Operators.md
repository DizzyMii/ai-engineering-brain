...
**The Dec 2027 / Aug 2028 dates are now settled at the legislative level, not merely proposed.** The Digital Omnibus on AI cleared both co-legislators — European Parliament (16 Jun 2026) and Council of the EU (29 Jun 2026) — and amends the AI Act's own compliance calendar; only Official Journal publication and the standard post-publication entry-into-force lag remain, expected in July 2026. An operator should still confirm the exact Official Journal publication date before hard-coding it into a compliance calendar, but the underlying policy outcome (the 2-year-plus deferral of Annex III/Annex I high-risk obligations) is no longer in question.
...
SMEs, including startups, get the *lower* of the fixed amount or the percentage for each tier (the inverse of the "whichever is higher" rule that applies to larger undertakings) — a deliberate ceiling to keep AI Act exposure proportionate for small deployers. (E3, statutory text, Art. 99.)

## Provider vs. deployer

Most enterprises deploying AI in or into the EU are **deployers**, not providers — they use a third party's model rather than building and placing it on the market. Deploying a third-party high-risk model into a high-risk use case still carries a distinct, non-trivial deployer duty stack under **Article 26**:
...
None of this requires *building* a model — it attaches purely to *using* one in a high-risk context, which is the operational surprise that catches enterprises assuming "we didn't build it, so it's not our problem."
...
An operator's practical exposure here is indirect: GPAI duties bind the model *provider* (OpenAI, Anthropic, Google, Meta, etc.), but an operator relying on that model inherits the need to obtain and retain the provider's documentation as part of its own Art. 26 due-diligence and human-oversight obligations.
...
- **"Operator" (AI Act) != "controller" (GDPR).** The two regulatory regimes use different roles and different tests; an entity can be a GDPR controller and an AI Act deployer simultaneously, and fines under each regime can stack independently for the same incident.
- **Reach is extraterritorial.** The Act applies where the AI system's *output* is used in the EU, regardless of where the provider or deployer is established — a US company serving EU users through a non-EU-hosted model is still in scope.
- **The Omnibus delay is legislatively final as of late Jun 2026** (Parliament 16 Jun, Council 29 Jun) — see the enforcement-timeline table above. Only the Official Journal publication date is still open; confirm it before setting an internal compliance-calendar deadline, but do not plan around the original 2 Aug 2026 date reverting.
- **Enforcement lag matters operationally.** GPAI obligations applied from 2 Aug 2025, but the AI Office's full enforcement powers (information requests, recalls, fines) only activate 2 Aug 2026 — a year-long gap between obligation and enforceability that some operators are treating as a soft grace period, which is a risk read, not a legal one.
