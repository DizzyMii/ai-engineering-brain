...
³ Getty dropped its primary UK training claim on **territorial** grounds — it could not establish that reproduction/storage occurred *in the UK* — so the court never decided whether training on protected images is infringing as a matter of law. Jurisdiction, not merits.
...
- **Provenance is the fault line.** Piracy loses even where training wins ([[Concept - The Pilot-to-Production Gap|deployment]] on a model trained on shadow-library data is inherited legal risk). Ask vendors how their training corpus was *acquired*, not just what it contains — the same provenance discipline that [[Concept - Deduplication at Scale|corpus curation]] applies for quality now applies for liability.
- **Output reproduction is the live risk for deployers, not training.** Your exposure is less "the model was trained on X" and more "the model *emitted* a near-verbatim X to my customer." Grounding, retrieval-with-citation, and output filtering reduce it — the same controls that limit [[Lore - Hallucination Liability Incidents|hallucination liability]].
- **Vendor indemnification is now a standard procurement ask.** Microsoft (Copilot Copyright Commitment), Google, Anthropic, and OpenAI offer copyright indemnities in varying forms and with varying carve-outs (they generally exclude cases where *you* disable guardrails or knowingly infringe). Read the carve-outs — this is a core input to [[Decision - Build vs Buy vs Wrap|build-vs-buy]].
- **[[Concept - Synthetic Training Data|Synthetic data]] and licensed data are the industry's structural response** — reducing dependence on scraped corpora is now a legal strategy, not only a data-wall strategy.
- **The EU handles this through a different lever** — GPAI training-data-summary and copyright-policy obligations rather than case law; see [[Reference - The EU AI Act for Operators]].
