...
> **Hook:** Target (mean) encoding replaces a high-cardinality category with that category's target mean. You get one dense, ordered numeric column, which [[Concept - Gradient Boosting|gradient-boosted trees]] split on far more efficiently than a wide one-hot block. Done naively, it's the most common Kaggle leakage bug: a row's own label bleeds into its own feature and the [[Concept - Statistical Rigor in Model Evaluation|cross-validation]] score becomes fiction. This is the out-of-fold, smoothed version that generalizes.

There are two independent guards. **Out-of-fold (OOF)** computes each training row's encoding from folds that *exclude* it, so no row sees its own target. **Bayesian smoothing** pulls categories seen only a handful of times toward the global mean, so a level with `n=2` rows doesn't get a wildly overconfident encoding. You need both. Either one alone still fails.
...
1. **The OOF split guards against leakage.** Fit `groupby(col)[target].mean()` on the whole training set and map it back, and every row's encoding contains that row's own `y`. For a category with one row the encoding *is* the label: `corr(cat_te, y)` shoots to ~1.0 and the model learns to read the answer off the feature. Encoding each fold from the *other* folds cuts that path, which is why `oof.iloc[val_idx]` is assigned from `enc` built on `tr_idx` only.

2. **The smoothing prior guards against variance.** `(n*cat_mean + m*global_mean)/(n+m)` is conjugate-style shrinkage (Micci-Barreca 2001). Without it, a level seen 3 times with all-positive labels encodes to 1.0, and the tree treats a noise spike as certainty. `m≈20` means "trust the category once you've seen it far more than 20 times; until then, trust the base rate." Tune `m` to the noise level, not the cardinality.

3. **Fit and transform are separate on purpose.** Training rows get the *OOF* encoding. Test/inference rows get a *single* mapping fit on the full training set. Mix them up, say by applying the full-train mapping to train rows too, and the leak comes back on the training split without any warning. The returned `mapping` dict is a fitted artifact you serialize and ship. It's the only thing that touches production, which matches how the encoder has to run.

4. **`fillna(global_mean)` handles the unseen category that naive code crashes on.** A category present at serve time and absent from training maps to `NaN`, and falling back to the prior is the correct, calibrated default. It isn't defensive padding. High-cardinality columns (user IDs, ZIP codes, SKUs) reliably turn up new levels in production.

**The trap this snippet does *not* fully solve:** OOF encoding protects you *within* the training set. Wrap the whole pipeline in an outer CV loop for model selection and the encoder has to be **re-fit inside each outer fold**. Fit it once on all data and the outer validation fold's targets have leaked into the encoding of every outer training fold. The general rule: any statistic learned from the target (encodings, priors, normalization by target-correlated quantities) is a model parameter and belongs inside the resampling boundary. [[Concept - Time Series Cross-Validation and Leakage]] has the temporal version.

**Another answer to the same problem:** CatBoost's *ordered target statistics* skip folds entirely. It draws a random permutation of the rows and encodes each row from the targets of the rows before it (an expanding-window mean). That's leakage-free online and sidesteps the fold-count/variance tradeoff. Same disease, different cure, and worth knowing when you reach for [[Breakdown - LightGBM|a boosting library]] and want native categorical handling.
