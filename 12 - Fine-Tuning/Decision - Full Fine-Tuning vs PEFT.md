...
> Default for the 80% case: **use LoRA (or QLoRA if VRAM-constrained).** Use full fine-tuning only when the domain shift is large, you have the hardware, you're deploying exactly one model, and you need the last few points of quality PEFT leaves on the table.
...
|---|---|---|---|---|
| Full fine-tuning | ~14GB weights + ~14GB grads + ~56GB fp32 master+moments ≈ 84GB → needs an 80GB A100 ([[Concept - GPU Memory Hierarchy]]) with offload or multi-GPU | Highest; the quality ceiling, measurably ahead on large domain shift (code, math) | Hard to roll back; moving every weight carries real [[Concept - Catastrophic Forgetting]] risk | Large domain shift, single deployed model, need the last few points |
| LoRA (16-bit) | Base ~14GB + adapter optimizer state in the low tens of MB | Near full FT on instruction/style adaptation; measurably lags on large shift ([[Concept - Why LoRA Underperforms Full Fine-Tuning]]) | Trivial: drop the adapter, the base is untouched; also regularizes against forgetting | Limited GPU budget, many task variants, want reversibility |
...
The memory numbers matter more than they look. Full fine-tuning's optimizer state alone (fp32 master weight plus two fp32 Adam moments, per [[Concept - Adam and AdamW]] under [[Concept - Mixed Precision Training]]) is what forces multi-GPU or offload; [[Reference - Memory Math for Transformers]] has the general formula. PEFT doesn't trim that arithmetic a little. It removes almost all of it, because the optimizer only tracks the adapter's parameters.

## The details that flip the decision

- **Large domain shift flips toward full fine-tuning.** Biderman et al. 2024 ("LoRA Learns Less and Forgets Less") found that on continued pretraining into code and math, LoRA underperforms full fine-tuning substantially. The low-rank update doesn't have the capacity to represent a large new skill or knowledge distribution. On instruction fine-tuning (a small shift from the base's existing capabilities) the gap between LoRA and full FT is small.
- **Deploying exactly one model with no task variants flips toward full fine-tuning.** PEFT's main advantage, many small adapters sharing one frozen base, doesn't apply when there's one deployment target and you can afford the hardware.
- **Limited GPU budget, many task variants, or a need for cheap rollback flip toward PEFT.** A frozen base plus swappable LoRA adapters holds many task-specific behaviors without N full copies of the model, and reverting a bad fine-tune means deleting an adapter file, not redeploying a multi-GPU job.
- **QLoRA vs. 16-bit LoRA vs. full FT is its own sub-decision inside "choose PEFT".** QLoRA saves the most memory at a real training-speed cost (dequantizing the 4-bit base on every matmul). 16-bit LoRA is better when you have the VRAM and want full training throughput. Full FT is worth its cost only when both points above (domain shift, single deployment) hold.
- **Non-obvious flip: LoRA isn't automatically faster per training step than full fine-tuning.** [[Deep Dive - LoRA]] barely changes forward-pass FLOPs, since the frozen weight matrix still gets multiplied through in full. The savings are optimizer, gradient and (with checkpointing) activation *memory*, not compute. If your bottleneck is GPU-hours and not GPU-memory, PEFT's advantage shrinks or disappears, so redo the decision on wall-clock cost instead of assuming.
