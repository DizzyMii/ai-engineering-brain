...
Theory is in [[Concept - Cost Engineering for LLM Applications]], and the atomic-limit pattern is from [[Concept - Rate Limiting and Quota Design]]. This snippet implements both on the cost axis. A gateway like [[Breakdown - LiteLLM]] ships this kind of enforcement built in, if you'd prefer not to run it yourself; the mechanics are the same either way.
...
- **Bill from `usage`; estimate only pre-flight.** `tiktoken` doesn't match every provider's tokenizer. It's close for OpenAI models and a rough guess for anything else, so using the estimate as the invoice figure silently mis-bills every non-OpenAI call. The provider's returned token counts are the only authoritative number. [[Gotchas - LLM Production Operations]] lists this mistake as a recurring incident.
- **One atomic Lua script, not `GET` then `INCR`.** "Check remaining, then increment" from application code has a race window. Two concurrent requests both read "under budget" before either writes, and both commit. That's the classic TOCTOU bug, and why [[Concept - Rate Limiting and Quota Design]] insists on an atomic op for any distributed limit. Doing check-and-increment in one Redis-side script closes the window however many app replicas are calling it.
- **Reserve on `max_tokens`, reconcile to actual.** You don't know the output length until generation finishes, so gating *before* the call means reserving pessimistically against the ceiling and truing up afterwards. It's the reserve-then-reconcile shape from token-based rate limiting, applied to dollars instead of request slots.
- **Pricing keyed by the dated snapshot id.** `"gpt-4o"` is a floating alias that can repoint to a differently priced model with no deploy on your side (see [[Concept - Model Lifecycle and Versioning]]). Keying the pricing map, and so every cost dashboard, on the dated id keeps cost attribution correct through a silent provider-side model swap.
