...
An Obsidian vault of **~660 interlinked notes on AI engineering**, covering the stack from the math substrate up through frontier deployment economics. Every domain is laddered **surface → core → advanced → frontier → unicorn** — from "what it is and why it exists" out to the tribal knowledge almost nobody writes down.

This is not a link dump or an awesome-list. It is a single connected graph with enforced structure: every note satisfies link minimums, every link resolves, every empirical claim in the applied domains carries an evidence tier, a named source, and a date.
...
| 13 | Evaluation | Trustworthy measurement and how benchmarks quietly break |
...
The vault is governed by a written constitution (`_SYSTEM/STANDARDS.md`) and enforced by tooling:
...
- **The Linking Law** — every note carries a minimum number of resolved links (12 for a Deep Dive, 8 for a Concept, down to 4 for a Reference), links into at least two other domains, and links up and down its ladder. `_SYSTEM/tools/linkcheck.py` verifies the whole graph: zero broken links, zero orphans, zero violations.
...
- **Temporal honesty** — fast-moving claims are date-stamped *(as of YYYY)*. Vault knowledge is current as of **mid-2026**; the notes that age fastest say so themselves and are expected to be wrong within 6–12 months on cost and reliability figures. Check dates before quoting.
- **Banned** — listicle framing, motivational filler, unexplained superlatives, untiered vendor numbers, padding. Mechanism, numbers, failure modes, named sources — or the section gets deleted.
...
Open the folder as a vault in [Obsidian](https://obsidian.md) (free; no plugins required). Start at `00 - Home/Home.md`, then either browse by domain MOC or take one of the guided ladders:
...
The notes are plain Markdown, so everything is readable on GitHub — but `[[wikilinks]]` only resolve inside Obsidian (a link target is always a note's filename), and the graph view is half the point. Read it in Obsidian if you can.
...
ShareAlike means the non-commercial restriction travels with every fork and adaptation — relicensing a derivative to strip it is not permitted. For commercial licensing, open an issue on this repository.
