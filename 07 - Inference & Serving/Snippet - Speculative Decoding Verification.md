...
**What it does:** implements the modified-rejection-sampling rule from [[Concept - Speculative Decoding]] — given target probs `p` and draft probs `q` over `k` drafted positions plus one bonus position, accepts drafted tokens one at a time and stops at the first rejection, resampling the replacement from the normalized positive residual. A second function drives thousands of trials and checks the emitted-token histogram against `p` directly, demonstrating the losslessness theorem rather than just asserting it. **Dependencies:** `numpy>=1.24` only — this is pure probability manipulation, not a model, so no tensor framework is needed. **Expected output:** printed mean accepted length (out of `k`) and a passing assertion that the empirical token histogram matches `p` to within a small tolerance.
...
- **`min(1, p/q)` paired with `max(0, p-q)` is the unique pairing that reconstitutes `p`, for any `q`.** Case-check the identity directly: when `p(x) >= q(x)`, `min(p,q) = q` and `max(0, p-q) = p-q`, summing to `p`; when `p(x) < q(x)`, `min(p,q) = p` and `max(0, p-q) = 0`, again summing to `p`. So `P(emit x) = q(x)·min(1, p(x)/q(x)) + P(reject)·[max(0, p(x)-q(x)) / P(reject)] = min(p(x),q(x)) + max(0, p(x)-q(x)) = p(x)` — the residual's normalizer, `P(reject) = sum_x max(0, q(x)-p(x))`, cancels exactly against the residual's own total mass, `sum_x max(0, p(x)-q(x))`, because both equal the same total-variation gap between `p` and `q`.
- **The draft ids are drawn from `q`, not fresh.** `draft_ids` in the test harness come from sampling `q_single` directly, matching what a real draft model would have produced. The `min(1, p/q)` formula is only valid conditioned on `x` already being a draw from `q` — testing against freshly-drawn `x'` independent of the draft's own samples would silently invalidate the whole derivation.
- **The bonus token is free because the target already computed it.** Position `k` in `p` comes from the same single forward pass that scored positions `0..k-1`; sampling it costs nothing extra, which is why full acceptance of `k` drafted tokens yields `k+1` tokens per target call, not `k`.
- **The empirical check accumulates *every* reached position's outcome, not just the last one emitted.** This is the detail that's easy to get wrong: it's tempting to test only `emitted[-1]` per trial, but that token is the resampled replacement whenever a rejection happened — and *conditioned on rejection*, the resample alone is drawn from `residual`, not from `p`; only the *mixture* of the accept branch and the reject branch reconstructs `p` (the identity above). Each position's own accept-or-resample decision is that full mixture and is independent of every other position's randomness, so accumulating every token any trial actually reaches (accepted draft tokens, a mid-sequence resample, or the bonus) into one histogram converges to `p`. Verify this distinction yourself: swap the accumulation loop back to `hist[emitted[-1]] += 1` and the assertion fails with an error around 0.2 — a fast, concrete way to feel why "the accept/reject mixture equals `p`" is a statement about a full position's decision, not about whichever slot a trial happens to end on.
