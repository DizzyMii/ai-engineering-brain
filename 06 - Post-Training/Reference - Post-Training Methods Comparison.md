...
1. **Models resident** = distinct full-size model copies held in accelerator memory at the same time during the training step (policy, frozen reference, reward model, value/critic). It's the main driver of the "relative compute" column. Byte-level accounting per model is in [[Reference - Memory Math for Transformers]].
2. **Relative compute** is a rough wall-clock multiplier over an equivalent SFT run on the same data volume. It excludes the one-time cost of RM training and depends on rollout/generation efficiency; a slow sampler inflates PPO/GRPO disproportionately. Figures as of 2026. Read them as order-of-magnitude, not benchmark numbers.
3. Reference-free methods (ORPO, SimPO) drop the KL anchor entirely. That saves a resident model but loses the built-in overoptimization brake DPO, IPO, KTO, and GRPO keep through the reference model.
