...
This is the operational trace through [[Deep Dive - The Pretraining Data Pipeline]]; each step below links to the concept note that explains *why* it works the way it does. Get the ordering wrong — score before dedup, mix before decontaminate — and you either waste compute or ship a contaminated corpus.

## Steps

**1. Fetch.** Pull the selected dumps from `s3://commoncrawl/` as raw **WARC** files, never WET. Expect roughly 90–100 TiB compressed per monthly dump (250+ TiB uncompressed). *Expected observation:* your object listing shows `.warc.gz` segments, not `.wet.gz`. *Deviation:* if only WET is available for a source, you have silently given up the extraction-quality lever — WET is Common Crawl's naive pre-extracted plaintext and still carries boilerplate.

**2. Extract.** Run [[Concept - Text Extraction from Web Pages]] over every WARC record with trafilatura or resiliparse, orchestrated through the DataTrove or Dolma toolkit. *Expected observation:* spot-read 100 random extracted docs — they should read as continuous article prose. *Deviation:* menu items, cookie banners, or "Subscribe now" fragments surviving into the text means the extractor or its config is wrong; this is the single highest-leverage step to get right, per RefinedWeb's central finding.

**3. Language ID.** Route or drop documents with fastText `lid.176`, typically keeping English at a probability threshold around p ≥ 0.65. *Expected observation:* the resulting language histogram roughly matches your target distribution (English will dominate — Common Crawl is ~44% English documents). *Deviation:* a flat or noisy histogram usually means extraction artifacts (mojibake, truncated text) are corrupting the LID signal, not that LID itself is broken.

**4. Filter.** Apply heuristic rules first (Gopher/C4-style: word count, mean word length, symbol-to-word ratio, terminal-punctuation checks — see [[Reference - Data Filtering Heuristics]]), then a quality classifier or LLM-annotator on the survivors, per [[Concept - Quality Filtering for Pretraining Data]]. *Expected observation:* survival rate lands in the 1–15% band of input tokens. *Deviation:* survival above ~50% means your filters are too weak (junk is getting through); survival below ~0.5% means you're over-filtering and narrowing the distribution — validate the choice against [[Decision - Choosing a Quality Filtering Strategy]] rather than trusting a filter's internal precision.

**5. Dedup + decontaminate.** Run MinHash-LSH per [[Concept - Deduplication at Scale]] (typically `num_perm=128`, similarity threshold ≈0.8) with text-dedup or DataTrove, **within each dump rather than globally** — cross-dump dedup has been shown to strip high-quality repeated content. Then run [[Concept - Training Set Decontamination]]: strip any document with n-gram (e.g. 13-gram) or substring overlap against your eval sets, which functions as targeted dedup against [[Concept - Benchmark Contamination]]. *Expected observation:* duplicate rate drops sharply and a per-benchmark overlap report shows ~0% overlap. *Deviation:* nonzero overlap on a benchmark you didn't decontaminate against means you're blind to it — this only catches benchmarks known at crawl time, never those released later.

**6. Assemble.** Apply your chosen [[Concept - Data Mixtures]] weights, run a global seeded shuffle across the mixed pool, tokenize with [[Concept - Byte-Pair Encoding]], pack sequences to the training `seq_len` with document separators, and write memory-mapped shards (Megatron `.bin`/`.idx`, WebDataset tar, or Mosaic MDS). *Expected observation:* token count matches the mixture's target budget, shard checksums are clean, and a decode-roundtrip on a random sample reproduces the source text exactly. *Deviation:* a mismatched token count means a mixture-weighting bug; a failed decode-roundtrip means a tokenizer/vocab mismatch between build time and training time — catch this before you touch [[Deep Dive - Anatomy of a Pretraining Run]] and commit GPU-hours.

## Verification

Before handing the corpus to a training job: (1) confirm final token count against the planned budget, (2) confirm shard integrity via checksums and a load-and-decode smoke test on a handful of random shards, (3) confirm the per-benchmark contamination report is clean, and (4) confirm the survival-rate and language-histogram sanity checks from steps 3–4 still hold on the assembled corpus, not just per-dump. Reproducibility check: the whole run should be re-derivable from a pinned manifest of tool versions, filter configs, and the shuffle RNG seed — if it isn't, you don't actually have a corpus, you have a one-off artifact.
