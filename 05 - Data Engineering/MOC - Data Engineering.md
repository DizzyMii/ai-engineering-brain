...
This domain covers the pipeline from the raw, messy internet to the tokens a model trains on. Where the text comes from (Common Crawl, licensed corpora, synthetic generation). What gets thrown out (quality filters, PII/toxicity scrubbing, deduplication, benchmark decontamination). How the surviving tokens get weighted, ordered and shipped into a run (data mixtures, curricula, sharding). The post-2020 data-centric thesis holds that at fixed compute, corpus quality explains more of the gap between models than architecture does. Every filtering choice is also a values choice: C4's blocklist deleted LGBTQ content, and Books3's shadow-library corpus triggered the industry's copyright reckoning. Coverage runs from what Common Crawl is up to two frontier questions: does recursive training on synthetic data collapse a model's tails, and do learned mixture weights transfer across scale? This domain sits upstream of [[MOC - Training at Scale]] (what the run is fed) and asks the same quality questions [[MOC - Post-Training]] asks about SFT/preference data, earlier and at far larger scale.
