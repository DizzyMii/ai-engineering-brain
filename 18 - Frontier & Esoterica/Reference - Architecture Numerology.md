...
Constants copied config-to-config across transformer families, and the mechanical reason each holds. Values marked *(as of 2026)* are lineage conventions, not laws.
...
The invariant is **FFN params $\approx 8\,d_\text{model}^2$**, not "4×". Gated units have three weight matrices (gate, up, down), so the width drops to $\tfrac{8}{3}$ to keep parameter count and FLOPs matched to the 4× baseline. The $\tfrac{8}{3}$ result then gets rounded to a hardware-friendly multiple (Llama-2-7B: $d_\text{ff}=11008$, a multiple of 256).
...
128 wins at scale. [[Deep Dive - FlashAttention|FlashAttention]] and [[Concept - Tensor Cores|tensor cores]] want the head dim to be a multiple of the matrix-multiply tile (8/16), and 128 balances kernel efficiency against the $1/\sqrt{d_h}$ softmax scaling. A $d_h$ that's too large inflates raw logit variance and feeds [[Concept - Attention Entropy Collapse|attention entropy collapse]].
...
$50304 = 128 \times 393$. The extra rows are never-predicted padding tokens, and the last tile of the $[\,d_\text{model} \times V\,]$ output projection is now full instead of half-empty. Karpathy's nanoGPT documents it as free throughput.
...
Most families sit in a narrow band. Too deep and thin is unstable to train (gradient/signal propagation); too wide and shallow underperforms at a fixed budget. Shape is a second-order knob next to total parameters and tokens ([[Deep Dive - The Transformer|the transformer]] has the block structure).
...
A larger base gives more [[Concept - Rotary Position Embeddings (RoPE)|RoPE]] dimensions wavelengths shorter than the target context, so they generalize instead of extrapolating out-of-distribution. "Just raise theta" is the most common context-extension knob, and the blunt alternative to [[Concept - RoPE Extrapolation and Context Extension|frequency-rescaling methods]] like YaRN.
...
| Weight decay | 0.1 | decoupled ([[Concept - Adam and AdamW|AdamW]]); matters for generalization |
...
The betas give it away: the framework default $(0.9, 0.999)$ is a vision/small-batch inheritance, and labs quietly switched $\beta_2$ to 0.95 for LLMs. It's documented folklore; [[Lore - Hyperparameter Folklore]] has the war stories behind these numbers.
...
The rule: **align every dimension to a power-of-two-ish tile so the hardware's systolic array stays full.** A model that trains "mysteriously slow" often has one dimension off a tile boundary. [[Reference - Memory Math for Transformers|The memory-math reference]] has the byte-level consequences; the soft-cap constants ($t\approx 50/30$) mentioned above are covered in the attention-stability note.
