...
> A map of ignorance, not a resolution. Each row is a starting point, and the last column is where the deeper work lives. Everything here is unsettled and volatile by design; the frontier moves *(as of 2026)*.
...
Zhang et al. 2017 ("Understanding deep learning requires rethinking generalization") states the problem cleanly. A standard net fits CIFAR-10 with *randomly permuted labels* to zero training error, so any generalization bound that depends only on the hypothesis class (VC dimension, Rademacher complexity of the architecture) is vacuous: the architecture that generalizes on real labels also memorizes noise. [[Concept - Double Descent]] and grokking *describe* the empirical curves (test error non-monotone in capacity; generalization arriving long after train loss saturates), but neither *predicts* test error from architecture + data + optimizer.

The partial answers are all incomplete. (a) SGD has an *implicit bias* toward low-norm / max-margin interpolants (provable for linear/logistic; Soudry et al. 2018). (b) Flatness of the minimum found correlates with generalization and connects to a minimum-description-length argument; see [[Concept - Mode Connectivity and Flat Minima]]. (c) Feature-learning vs kernel (NTK) regimes explain some of it, not all. No theory takes a `(model, dataset)` and returns a generalization gap.

## 2. Why does in-context learning work?

Three live hypotheses, each with real evidence, none complete:

- **Circuit account.** Olsson et al. 2022 ("In-context Learning and Induction Heads") tie the phase change in ICL ability to the formation of induction heads that prefix-match and then copy. The ICL loss drop and induction-head formation land on the same training step.
- **Implicit optimization.** von Oswald et al. 2023 show a transformer forward pass can *implement* one or more gradient-descent steps on the in-context examples, which would make ICL learning in the activations.
- **Bayesian inference.** Xie et al. 2021 frame ICL as implicit Bayesian inference. The prompt selects a latent concept already in the pretraining distribution, so ICL retrieves a pretrained skill and learns nothing new.

It's hard to separate empirically whether a model learns a truly novel function in context (evidence for optimization) or selects a pretrained skill (evidence for Bayesian retrieval). This is tangled up with [[Concept - The Emergent Abilities Debate]], since whether ICL "emerges" is partly a measurement question.

## 3. Is hallucination fixable in-parameter, or does it need retrieval?

Models are reasonably *calibrated at the token level*, meaning next-token probabilities track empirical frequencies, yet they state false *facts* confidently. Kalai et al. 2025 ("Why Language Models Hallucinate") argue much of this is an incentive artifact: pretraining and most benchmarks reward a confident guess over "I don't know," so the loss-minimizing policy bluffs on the tail of rare facts. That's a training/eval-design lever and no mystery. It still doesn't tell you whether factuality can be made reliable *in the weights*.

The obstacle is that parametric knowledge is stored directionally and lossily. Per the reversal curse, a model trained on "A is B" may not know "B is A," so some facts can't be extracted even when "present." That's a standing argument that retrieval is essential, not a temporary crutch. Open: whether some scaling or training recipe closes the factuality gap in-parameter, or grounding is needed for good.

## 4. The interpretability ceiling: superposition and "dark matter"

Networks pack more features than they have neurons by storing them as near-orthogonal directions, which are only separable when active features are sparse (Elhage et al. 2022, "Toy Models of Superposition"). Sparse autoencoders trained on the residual stream (Bricken et al. 2023; Templeton et al. 2024, "Scaling Monosemanticity" on Claude 3 Sonnet) recover thousands to millions of monosemantic features. That's real progress, but a large share of activation variance stays unexplained as SAE "dark matter," and *enumerative safety* (list every feature, verify none is dangerous) is unproven at scale. The open question matters a lot for safety: is full mechanistic interpretability tractable at all, or is there an irreducible residual no dictionary recovers? People doing this work run into a long list of practical pitfalls well before the dark-matter question; see [[Gotchas - Interpreting Model Internals]].

## 5. Where does scaling end?

Chinchilla-optimal training wants roughly 20 tokens per parameter, and frontier runs are token-hungry against a finite supply of high-quality web text. Muennighoff et al. 2023 ("Scaling Data-Constrained Language Models") measured the returns to *repeating* data: up to ~4 epochs is nearly as good as fresh tokens, then value falls off sharply. Compute can substitute for data only so far. Manufacturing data risks [[Concept - Model Collapse from Synthetic Data]]; Shumailov et al. 2024 (Nature, "AI models collapse when trained on recursively generated data") show recursive training on model output erodes the distribution's tails generation by generation. Meanwhile test-time compute (the o-series reasoning wave, 2024–2025; see [[Concept - Reasoning Training and Long Chain-of-Thought]]) opened a scaling axis orthogonal to pretraining FLOPs. Open: whether the pretraining curve bends before the tokens run out. The practical framing is in [[Concept - The Data Wall]] and [[Concept - Data-Constrained Scaling Laws]].

## 6. Continual learning without catastrophic forgetting

No production recipe lets a deployed frontier model absorb new facts *online* without a full (re)train or a retrieval crutch. Naive fine-tuning on new data causes catastrophic forgetting (McCloskey & Cohen 1989; French 1999): the new gradient overwrites weights that encoded old skills. The mitigations, Elastic Weight Consolidation (Kirkpatrick et al. 2017), experience replay and adapter/LoRA isolation, each trade plasticity against stability, and none is clean at frontier scale. Targeted knowledge editing (ROME, MEMIT) writes single facts but is brittle and directional (the reversal curse again). So "just fine-tune it on the new docs" is rarely the right answer, and RAG persists.

## 7. The measurement problem

We can't cleanly state what a model can do. This is arguably the most operationally damaging open problem, because every other claim rests on it. Three failures compound. (a) The *metric* can manufacture the capability curve: Schaeffer et al. 2023 showed exact-match/multiple-choice thresholds create apparent emergence that disappears under smooth metrics. (b) [[Concept - Benchmark Contamination]] leaks test items into pretraining and inflates scores. (c) [[Concept - Goodhart's Law in Model Evaluation]] guarantees any headline benchmark gets gamed once it matters commercially. Evaluation (domain 13) owns the methodology; in practice the problem stays unsolved, so read "SOTA on X" together with the harness, the contamination controls and the confidence interval.

---

This reference is a map, refreshed as the frontier moves. For where the research actually surfaces (papers, lab blogs, threads that update these rows), see [[Reference - Where Real AI Knowledge Lives]].
