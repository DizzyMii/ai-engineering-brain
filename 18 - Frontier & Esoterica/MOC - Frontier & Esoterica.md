...
This domain owns the phenomena that don't fit cleanly into any other domain's narrative: training dynamics that defy monotonic-loss intuition (grokking, double descent, the lottery ticket), attention and softmax mechanics that misbehave under scale or long context, tokenization pathologies that corrupt a model's number sense and vocabulary, and the open problems and folklore the field runs on when theory hasn't caught up with practice. It matters because these are the notes a staff engineer reaches for when a model does something the textbook doesn't predict — a loss spike with no obvious cause, a context-extension trick that works but nobody can fully explain why, a token that makes the model hallucinate for no traceable reason. Much of what's here sits at the edge between established mechanism and open research question, which is why several notes are explicitly frontier or unicorn tier: documented but not fully understood, or understood but rarely written down outside a lab's internal Slack. Treat this domain as connective tissue for the rest of the vault — its notes routinely reach into architectures, training, and post-training to explain why a phenomenon appears where it does.

## Start here

- **Surface** → [[Lore - Machine Learning Is Alchemy]] — Rahimi's 2017 NeurIPS speech vs. LeCun's rebuttal, the debate that frames why this whole domain runs on folklore as much as theory.
