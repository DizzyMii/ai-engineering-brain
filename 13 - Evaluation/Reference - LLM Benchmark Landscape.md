...
Numbers here are approximate and move with every frontier release. Treat all of it as *(as of 2026)* and re-check before quoting it in a launch deck. Rows are grouped by capability, but the format/scoring column is what decides whether two numbers are comparable ([[Concept - Benchmark Taxonomy]] explains why). Run any cell through [[Checklist - Trusting a Benchmark Number]] before trusting it.
...
| Humanity's Last Exam | Mixed MCQ + generation, expert-authored | Thousands of items across many disciplines | Near 0% by design | Expert-level per item | Deliberately far below saturation at launch (2025) | Built to outlast saturation, so "SOTA" is a moving, low number by design |
...
| MuSR | Generate + parse, multi-step reasoning | Hundreds of long-form reasoning items | Low | High | Not saturated | Long contexts make cost per item high compared with MCQ benchmarks |
...
|---|---|---|---|---|
| GSM8K | Generate + parse, exact-match final number | ~1.3k test items | Saturated, ~95%+ since 2024 | On the web since 2021, so presumptively contaminated (see [[Concept - Benchmark Contamination]]) |
...
| AIME | Generate + parse, exact numeric answer | ~30 items/year | High variance run to run | One question is ~3.3% of the score; most reported AIME gaps between models aren't statistically significant |
...
| Benchmark | Format / scoring | Size | SOTA & saturation (2026) | Headline flaw |
|---|---|---|---|---|
| HumanEval | Execution-based pass@k | 164 hand-written problems | Saturated | On the web for years and heavily contaminated; also small enough that a handful of items dominate the score |
...
| BigCodeBench | Execution-based, function-call-heavy tasks | ~1.1k tasks | Meaningfully below HumanEval-style scores | Tests library/API use as well as algorithmic code; harder to compare across papers |
| SWE-bench / SWE-bench Verified | Execution-based, fail-to-pass + pass-to-pass tests | 2,294 original / 500 Verified (human-filtered) | ~2% (2023) → 60–70%+ on Verified (2025–26) | Measures the scaffold+model system, not the base model alone; see [[Breakdown - SWE-bench]] |
...
| AlpacaEval 2.0 | LLM-judged, length-controlled win-rate | Several hundred instructions | No fixed ceiling; win-rate is relative to a baseline model | Raw (non-length-controlled) version was gamed by verbosity before the LC fix shipped |
| Arena-Hard-Auto | LLM-judged, pairwise vs. baseline | ~500 curated hard prompts | More separable than MT-Bench, not saturated | Inherits whatever biases the judge model has; see [[Concept - LLM-as-Judge]] |
...
| Benchmark | Format / scoring | Size | SOTA & saturation (2026) | Headline flaw |
|---|---|---|---|---|
| RULER | Generate + parse, synthetic long-context tasks | Configurable context lengths, commonly tested to 128k+ | Frontier models degrade well before their advertised max context | "Advertised" and "usable" context length diverge, and RULER exists to expose the gap |
...
| MMMU | Mixed MCQ + generation, multimodal | Roughly 11.5k college-level questions across ~30 subjects | Below human-expert level | Blends modalities and question types into one score, which hides the weak modality |

## Footnotes

- **Random / human baselines** matter. A bare percentage without them can't be evaluated; see [[Concept - Benchmark Taxonomy]].
- **HellaSwag** and **ARC-Challenge** label noise means neither ceiling is 100%, even in principle.
...
- **AIME**'s ~30 items/year means single-question granularity (~3.3%). Always report the item count next to any AIME percentage.
