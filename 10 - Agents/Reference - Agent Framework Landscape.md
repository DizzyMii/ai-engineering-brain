...
|---|---|---|---|---|---|---|---|---|
| LangGraph | Explicit graph/state machine | Yes (subgraphs, supervisor patterns) | Yes | Native interrupt/resume | Durable, first-class checkpointing | None (any model provider) | Python, JS | Long-running or resumable workflows needing explicit branching and replay |
...
Most rows in the table above have the model emit a JSON tool call per action (see [[Concept - Tool Use and Function Calling]]), which the harness parses and executes. smolagents and the broader CodeAct line instead have the model emit a snippet of Python that is executed directly — one action can chain multiple operations (call a tool, inspect the result, call another tool) without a full model round-trip per step. This cuts latency and token overhead for multi-step actions, but it requires running arbitrary generated code, which means a real sandbox is non-negotiable (see [[Checklist - Sandboxing an Agent]]) rather than optional hardening.

## DSPy is orthogonal

DSPy does not compete with the other rows on control paradigm because it isn't a runtime loop at all — it's a compiler that optimizes the prompts (or weights) of the modules inside an agent program. You can wrap a LangGraph node, a CrewAI task, or a raw tool-use loop in a DSPy module and let it tune the module's prompt against a metric; DSPy answers "what should the prompt say" while the other frameworks answer "what runs when."

## The raw-API baseline

For a single agent with one to three tools and a bounded task, the framework overhead frequently costs more debugging time than it saves — a hand-rolled loop against the raw provider SDK (see [[Playbook - Building a Tool-Use Agent from Scratch]]) is often the correct 2026 default, with a framework adopted only once a concrete need appears (explicit state/branching, durable resume, or heavy multi-agent coordination). Reach for LangGraph or Google ADK when you need durable checkpointing; reach for the OpenAI Agents SDK or Claude Agent SDK when you want provider-native handoffs and guardrails without building them yourself.

## Volatility warning

This table is a snapshot, not a contract. In the last two years alone: OpenAI's Swarm (experimental) was superseded by the Agents SDK; the MCP transport layer moved from HTTP+SSE to Streamable HTTP in the 2025 spec revision (see [[Concept - Model Context Protocol (MCP)]]); and several frameworks not listed here (rows come and go with real adoption). Treat every "best-fit" claim as date-stamped to 2026 and re-verify before a production commitment — the actual choose-one decision logic, including what flips the default, lives in [[Decision - Choosing an Agent Framework]], not in this table.
