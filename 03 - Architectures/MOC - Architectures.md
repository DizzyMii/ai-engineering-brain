...
This domain owns the block diagram: how attention, feed-forward layers, normalization, positional encoding, and their alternatives (state-space models, linear attention, convolutions) are wired into a model that actually trains and serves. It sits below training procedure and above pure math — the concern here is tensor shapes, memory layout, and the mechanism of each block, not how the weights get learned or the cluster that runs them. It matters because architecture choices set constant factors everywhere downstream: head geometry decides KV-cache bytes per token, normalization placement decides whether a run at scale diverges, and the sequence-mixer choice trades the quadratic attention wall against recall quality. Most of the field's real engineering judgment — MHA vs. GQA vs. MLA, dense vs. MoE, full attention vs. hybrid SSM — lives in this domain as the notes with the sharpest tradeoff tables.

## Start here

- **Surface** → [[Concept - Encoder-Decoder and Decoder-Only Architectures]] — the taxonomy (encoder-only, encoder-decoder, decoder-only) that tells you which shape every other note in this domain assumes.
- **Core** → [[Deep Dive - The Transformer]] — the full block-by-block walkthrough (attention → FFN → residual → norm, stacked) that every other note in this domain either extends or replaces one piece of.
...
- **Frontier** → [[Decision - Choosing a Sequence Mixer]] — the live 2025-2026 debate (full attention vs. sliding-window vs. SSM vs. hybrid) stated as an actual decision with a tradeoff matrix.
...
- [[Concept - Multi-Head Attention Variants (MHA MQA GQA MLA)]] — the KV-head-sharing spectrum from MHA to MQA to GQA to DeepSeek's low-rank MLA, and the exact bytes-per-token each saves.
...
- [[Breakdown - Mixtral 8x7B]] — Mistral's 8-expert, top-2-routed model reverse-engineered: 47B total parameters, ~13B active per token, and what that ratio actually buys.
...
- [[Gotchas - Mixture of Experts]] — router collapse, expert-capacity overflow, and the load-balancing-loss tuning that keeps all experts actually used.
...
- [[Concept - The Residual Stream]] — the additive channel every sublayer reads from and writes to, and why its growing norm across depth is the thing normalization placement is actually managing.
...
- [[MOC - Training at Scale]] — what it actually takes to get one of these architectures to converge at billions of parameters: parallelism, mixed precision, and the failure modes normalization placement is meant to prevent.
