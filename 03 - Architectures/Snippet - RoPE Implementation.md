...
**What it does:** builds the RoPE frequency cache (`inv_freq`, `cos`, `sin`), rotates `Q` and `K` with the `rotate_half` trick, and checks empirically the property the scheme exists for: the dot product of a rotated query at position $m$ and a rotated key at position $n$ depends only on $(m-n)$. It's a runnable version of the math in [[Concept - Rotary Position Embeddings (RoPE)]]. Read that note for the derivation and this file for the tensor ops.
...
1. **Only `Q` and `K` get rotated, never `V`.** Position should shape *which* keys a query attends to. It shouldn't be baked into the *content* being retrieved. Rotate `V` and the attention output itself rotates with absolute position, when position should only act as a relative-distance effect on the attention weights.
2. **`rotate_half` splits the vector in half instead of interleaving even/odd indices.** The two conventions for pairing up `d_head` dimensions are mathematically equivalent in theory and bit-incompatible. This snippet uses the half-split (`rotate_half`) convention that GPT-NeoX-style code and most modern open-weight checkpoints use. Port weights from a checkpoint trained with the *other* (interleaved) convention into this code and you get a model that runs, generates plausible text, and is silently wrong. That one mismatch is the most common RoPE bug in practice; see [[Gotchas - Implementing Attention]].
3. **The relative-position check rotates one fixed `(q0, k0)` pair by every position, with no random per-position vectors.** Random `q`/`k` at each of two positions confirms nothing about the relative-position *property*. Hold content fixed and vary only where you rotate it to; then a constant dot product across varying `m` (fixed offset) is proof, not coincidence.
4. **`base` is a plain function argument, not hardcoded.** Every context-extension technique (Position Interpolation, NTK-aware scaling, YaRN) works by changing what goes into this one line: either the position indices or `base`/`theta` itself. Isolating it here turns those techniques into a one-line change instead of a rewrite; see [[Concept - Context Length Extension]] and the extrapolation folklore in [[Concept - RoPE Extrapolation and Context Extension]].
