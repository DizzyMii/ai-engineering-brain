...
*Parameter-count and FLOP formulas for [[Deep Dive - The Transformer]]'s block structure, plus a dimension table for real models. Numbers date-stamped (as of 2026). Full training-memory accounting (activations, optimizer state, KV cache) is in [[Reference - Memory Math for Transformers]].*
...
| Embedding | $vocab \cdot d_{model}$ | usually weight-tied to the unembedding (Press & Wolf 2017), so count once |
...
The $12d^2$ is $4d^2$ attention $+\ 8d^2$ FFN. GQA/MLA shrink the attention term. A GLU FFN at the standard $d_{ff}\approx(8/3)d_{model}$ rescale keeps the FFN term near $8d^2$ despite the extra matrix. [[Concept - Mixture of Experts Architecture|MoE]] multiplies the FFN term by the expert count for *total* params but not *active* params, and that decoupling is the point of sparse routing.
...
At typical pretraining sequence lengths (2k–8k) the FFN term dominates total FLOPs. Past roughly $N_{seq}\sim d$ tokens the quadratic attention term overtakes it. That crossover motivates [[Concept - Sparse and Sliding-Window Attention]] and every long-context serving optimization.

## Table 3: model configuration comparison (as of 2026)

Dimensions only. Lineage and licensing for each row are in [[Reference - Model Genealogy]].
...
- **`head_dim` comes from folklore, not a derivation.** 64 and 128 are essentially the only two values real models use (GPT-3's 96×128 vs. LLaMA's 32×128). No theoretical result pins this down. The convention stuck because it plays well with tensor-core tile sizes.
- **$d_{ff}$ gets rounded to a hardware-friendly multiple** after the $4d$ or $(8/3)d$ rescale. That's why entries like 11,008 and 14,336 look arbitrary instead of being exact multiples of $d_{model}$.
- **Vocab size has grown an order of magnitude in five years**: 32k ([[Concept - Byte-Pair Encoding|LLaMA-2's BPE]] tokenizer, English-centric) → 128k (LLaMA-3, more languages + code) → 150k–256k (Qwen2.5, Gemma 2, heavier multilingual coverage). Both the embedding parameter count and the final-layer logit FLOP cost go up with it.
- **"Total vs. active" only diverges for MoE rows** (Mixtral, DeepSeek-V3). Every dense row has total = active by definition. [[Decision - Dense vs Mixture-of-Experts]] is framed entirely around that gap.
