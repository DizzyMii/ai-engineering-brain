...
*Pin every row to an exact release.* Llama 2, 3, and 4 carry materially different terms under the same family name — "Llama's license" is not a single object.
...
These three are independent axes, not a spectrum — a model can have maximally open weights (MIT) while disclosing zero data (DeepSeek), or vice versa (a research release under a restrictive academic license with a fully public corpus).

## The OSAID fight

The Open Source Initiative's Open Source AI Definition (OSAID 1.0, Oct 2024) requires "sufficient" disclosure of training data composition — provenance, characteristics, and how to obtain or reconstruct it — for a model to be called open source. Meta disputes this because Llama satisfies none of it: no data disclosure, a behavioral Acceptable Use Policy, and the MAU clause. Under OSAID's letter, *no major "open" LLM from a commercial lab currently qualifies as open source* — only data-transparent research releases like OLMo and Pythia do. This is an active, unresolved definitional fight as of 2026; "open" on a model card and "open" per OSAID are different claims.

## Enforceability reality

A license is a promise enforceable against a counterparty who signed up to it — typically a company with lawyers, revenue, and a reputation. It does nothing against an anonymous BitTorrent seed. Once weights leak (see [[Lore - The LLaMA Leak]]), the license text still governs *reputable* downstream commercial use, but the leaked copy itself is unrecallable and untraceable. This is why licenses function less as technical access control and more as a liability perimeter around companies willing to be sued.
...
See [[Checklist - Vetting an Open-Weights Model for Production]] for the full production go/no-go, of which licensing is one stage.
